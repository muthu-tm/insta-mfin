...
class CollectionReceipt {
  Future<void> generateInvoice(User _u, Payment _p, Collection _c) async {
    ...
    final PdfGrid grid = await getGrid(_p, _c);
  ...
  Future<PdfGrid> getGrid(Payment _p, Collection _c) async {
    ...
    headerRow.cells[2].value = 'Installment No';
  ...
  void addRow(String pID, DateTime date, int ins, int paid, int pending, int amount,
      int tPending, int upcoming, PdfGrid grid) {
    final PdfGridRow row = grid.rows.add();
    row.cells[0].value = pID;
    row.cells[1].value = DateUtils.formatDate(date);
    row.cells[2].value = ins.toString();
    row.cells[3].value = paid.toString();
    row.cells[4].value = pending.toString();
    row.cells[5].value = amount.toString();
    row.cells[6].value = tPending.toString();
    row.cells[7].value = upcoming.toString();
  }
